...
## (STEP1-2) Command R+に熱中症環境保健マニュアルの内容を回答させよう
...
## (STEP3-1) Claudeに画像の時刻表をデータ化させよう
...
## (STEP3-2) ClaudeにWebアプリを作成させよう

### Claudeにプログラムを作成＆実行させよう
...
## (STEP7-1) 熱中症対策を回答してくれるチャットボットを作成してみよう
...
- ナレッジ(知識)を作成
  ...
  - 「チャンク設定」は「自動」のまま
  - 「インデックスモード」は「高品質」のまま
  - 「埋め込みモデル」を「embed-multilingual-v3.0」に変更
  - 「検索設定」は「ハイブリッド検索」に変更
  - 「再ランクモデル」を「rerank-multilingual-v3.0」に変更
  - 「保存して処理」をクリック
