...
- 左メニューの「作成」をクリック。プロンプト欄の左上が「ビデオ」になっていない場合は「ビデオ」を選択する
  - (1) テキストのみから動画を生成
...
```
嵐の空を巨大なドラゴンが飛んでいる。
翼を大きく羽ばたかせながら雲を突き抜ける。
口から激しい炎を吐き、火の粉が空中に広がる。
カメラはドラゴンを追いかけるように高速で飛行。
シネマティックな映画のような雰囲気。
ドラゴン, 炎, 空中戦, 雲, ファンタジー, ダイナミック, シネマティック
```
...
巨大な滝から大量の水が落ちている。
水しぶきが空中に舞い、虹がかかる。
カメラはドローンのように滝の上から下へ滑らかに降下。
自然ドキュメンタリーのような迫力。
滝, 水しぶき, 虹, ドローンショット, 自然, ダイナミック
```
...
```
未来都市の中で巨大ロボットが立っている。
装甲が動きながら形が変わり、戦闘モードに変形。
内部から光が溢れ、金属パーツが高速で組み替わる。
カメラがロボットの周囲を回り込む。
SF映画のような迫力。
巨大ロボット, 変形, メカ, 未来都市, SF, シネマティック
```
...
[<img src="https://github.com/OkinawaOpenLaboratory/ool-tech-connect/raw/main/MediaFiles/locomotive.jpg" width="40%">](https://github.com/OkinawaOpenLaboratory/ool-tech-connect/raw/main/MediaFiles/locomotive.jpg)　locomotive.jpg

```
古い蒸気機関車が停車している駅のホーム。
白い蒸気を上げながらゆっくり発車し始める。
蒸気が空に広がり、車輪が動き出す。
カメラは横から列車を追いかけるようにゆっくり移動。
レトロでノスタルジックな映画の雰囲気。
蒸気機関車, レトロ, 駅のホーム, ノスタルジック, シネマティック, フィルム風
```

◆海中を泳ぐウミガメ

[<img src="https://github.com/OkinawaOpenLaboratory/ool-tech-connect/raw/main/MediaFiles/turtle.jpg" width="40%">](https://github.com/OkinawaOpenLaboratory/ool-tech-connect/raw/main/MediaFiles/turtle.jpg)　turtle.jpg

```
透明な海の中を大きなウミガメがゆっくり泳いでいる。
周囲には小さな魚の群れ。
水面から差し込む太陽光が海中を照らしている。
カメラはウミガメの横をゆっくり追従して泳ぐ。
穏やかで美しい水中映像。
海中, ウミガメ, 魚の群れ, 水中映像, リラックス, 自然, シネマティック
```

◆古代遺跡の探索

[<img src="https://github.com/OkinawaOpenLaboratory/ool-tech-connect/raw/main/MediaFiles/stone_temple.jpg" width="40%">](https://github.com/OkinawaOpenLaboratory/ool-tech-connect/raw/main/MediaFiles/stone_temple.jpg)　stone_temple.jpg

```
砂漠の奥にある古代遺跡の内部。
考古学者がランタンを持って壁の彫刻を調べている。
光が遺跡の模様を浮かび上がらせる。
カメラはゆっくり遺跡の奥へ前進。
冒険映画のような神秘的な雰囲気。
古代遺跡, 考古学者, 探索, 砂漠, 冒険, 神秘的, シネマティック
```
...
- 「動画」、「フレーム」、「16:9」、「x1」「Veo 3.1-Fast」を選択
  - (1) テキストのみから動画を生成
    - (1-1) 「何を作成しますか？」と書かれたプロンプト欄に(STEP1-1)のプロンプト例をコピペ
      - 「Prompt must be provided」という警告がでる場合はプロンプトを選択して、プロンプト入力エリアにドラッグアンドドロップ
    - (1-2) 「→」アイコンをクリック
  - (2) 画像とテキストで動画を生成
    - (2-1) 画像をダウンロードしておく
    - (2-2) 「開始」とかかれた部分をクリック
    - (2-3) 「アセットを検索する」と書かれたプロンプト欄の右にある「アップロード」アイコンをクリックして、ダウンロードしておいた画像をアップロード
    - (2-4) 「何を作成しますか？」と書かれたプロンプト欄に(STEP1-1)のプロンプト例をコピペ
    - (2-5) 「→」アイコンをクリック
- 動画が生成されたら、クリックで動画が再生される
  - 動画にマウスを合わせると表示される右上の「↓」アイコンをクリックすれば動画をダウンロードできる
  - 右上のユーザーアイコンをクリックして「マイライブラリ」をクリックすると動画の生成履歴が閲覧できる

※ 1動画生成に20クレジット必要。(無料ユーザーには初回100クレジット＋毎日50クレジット与えられる)
...
- 左ペインの「新モデル」エリアの「AIモデル」を「Meshy 5」へ変更
  - ポーズとライセンスは変更不要
...
- 気に入ったオブジェクトを選んでクリックし、「ポリゴン数」が「固定された」になっており、「テクスチャを生成する」が「はい」になっているのを確認して「確認する」をクリック
- **※3分ほどでテクスチャが生成される**
...
- 左メニューの「Playground」をクリック
  - 左メニューが表示されていない場合は、左上の「三」(Toggle navigation menu)アイコンをクリックして表示させる
- 中央の「Text to Speech」を選択後、「Gemini 2.5 Pro Preview TTS」をクリック
...
- 「無料でアバター動画を生成」あるいは「今すぐ無料動画作成→」をクリック
  - 右上の「無料のAIビデオを作成します→」をクリックしてもよい
